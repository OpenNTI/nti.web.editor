...
function isStyleAllowed(style, context, shouldDisableForState) {
	...
	return (
		plugins?.allowedInlineStyles?.has(style) &&
		!readOnly &&
...
export default function StyleButton({
	...
	const isAllowed = isStyleAllowed(
		style,
		editorContext,
		shouldDisableForState
	);
	...
	const cls = cx('draft-core-style-button', className, {
		[activeClassName]: isCurrent,
		[disabledClassName]: !isAllowed,
		plain,
	});
