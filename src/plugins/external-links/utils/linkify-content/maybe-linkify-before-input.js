...
export default function maybeLinkifyBeforeInput (chars, editorState, allowedInBlockTypes) {
	...
	let newEditorState = EditorState.push(editorState, newContent, CHANGE_TYPES.INSERT_CHARACTERS);

	newContent = applyLinkToContent(link, newContent);
	newEditorState = EditorState.push(editorState, newContent, CHANGE_TYPES.APPLY_ENTITY);
