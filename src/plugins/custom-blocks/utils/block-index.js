...
export function getIndex(contentBlock, editorState, blockFilter) {
	...
	for (let block of blocks) {
		...
		if (
			!isFocusablePlaceholder(block) &&
			(!blockFilter || blockFilter(block, editorState))
		) {
			count += 1;
		}
	}
...
const isPlaceholder = block =>
	isFocusablePlaceholder(block) ||
	(block.getType() === BLOCKS.UNSTYLED && block.getText().trim() === '');
...
export function isLast(contentBlock, editorState) {
	return isFirstNonPlaceholder(
		contentBlock,
		editorState.getCurrentContent().getBlocksAsArray().reverse()
	);
}
