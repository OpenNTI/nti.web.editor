...
export default function setBlockData (block, data, useEntity, editorState) {
	...
	if (useEntity) {
		...
		newContent = ContentState.createFromBlockArray(newContent.getBlocksAsArray(), newContent.getEntityMap());
	} else {
		newContent = Modifier.mergeBlockData(content, selection, new ImmutableMap(data));
	}
