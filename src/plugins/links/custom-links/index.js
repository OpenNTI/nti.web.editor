...
export const create = () => {
	...
	const LinkWrapperInstance = props => (
		<LinkWrapper {...props} store={store} />
	);
	const LinkOverlayInstance = props => (
		<LinkOverlay {...props} store={store} />
	);
	...
	return {
		...
		onChange(editorState) {
			...
			wait().then(() => store.setItem(SelectedEntity, entityKey));
			...
			return {
				...
				toggleLink(link) {
					...
					if (selectedEntity) {
						...
						let newContent = createLinkEntity.createCustomLink(
							content,
							href || ''
						);
