...
export default function findLinks(content) {
	const entitiesByBlock = content.getBlocksAsArray().reduce((acc, block) => {
		const key = block.getKey();
		const entities = block
			.getCharacterList()
			.toJS()
			.reduce(appendToSet, new Set());
	...
	return Object.entries(entitiesByBlock).reduce(
		(acc, [blockKey, entities]) => {
