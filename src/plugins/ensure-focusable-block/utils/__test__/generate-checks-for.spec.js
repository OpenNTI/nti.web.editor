...
describe('generateChecksFor', () => {
	describe('isFocusable', () => {
		test('True if not in around or between', () => {
			...
			expect(
				isFocusable({ type: BLOCKS.ORDERED_LIST_ITEM })
			).toBeTruthy();
			expect(
				isFocusable({ type: BLOCKS.UNORDERED_LIST_ITEM })
			).toBeTruthy();
		});
	...
	describe('shouldInsertAround', () => {
		test('False if not in around', () => {
			...
			expect(
				shouldInsertAround({ type: BLOCKS.ATOMIC }, null)
			).toBeFalsy();
		});
		...
		test('False if prev is the same type', () => {
			...
			expect(
				shouldInsertAround({ type: BLOCKS.CODE }, { type: BLOCKS.CODE })
			).toBeFalsy();
		});
	...
	describe('shouldInsertBetween', () => {
		test('False if not in before', () => {
			...
			expect(
				shouldInsertBetween({ type: BLOCKS.CODE }, null)
			).toBeFalsy();
		});
	...
	describe('shouldInsertBefore', () => {
		test('True if no prev block and not focusable', () => {
			...
			expect(
				shouldInsertBefore({ type: BLOCKS.CODE }, null)
			).toBeTruthy();
		});
