export default function generateChecksFor (around, between) {
	const isFocusable = (block) => !around.has(block.type) && !between.has(block.type);
	...
	const shouldInsertAround = (block, prev) => {
		return around.has(block.type) && block.type !== prev.type && !isFocusable(prev);
	};
	...
	const shouldInsertBefore = (block, prev) => {
		return (!prev && !isFocusable(block)) || shouldInsertAround(block, prev) || shouldInsertBetween(block, prev);
	};

	return {isFocusable, shouldInsertBefore, shouldInsertAround, shouldInsertBetween};
}
